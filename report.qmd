...
avg_price <- price_long |>
  dplyr::group_by(year) |>
  dplyr::summarise(avg_price = mean(price, na.rm = TRUE), .groups = "drop")

p1 <- ggplot(avg_price, aes(year, avg_price)) +
...
p2 <- ggplot(ev_top10, aes(x = reorder(state_full, ev_registrations), y = ev_registrations)) +
  geom_col() + coord_flip() +
...
p3 <- ggplot(compare_2023, aes(x = price_2023, y = ev_registrations)) +
...
p4 <- ggplot(map_ev, aes(long, lat, group = group, fill = ev_registrations)) +
