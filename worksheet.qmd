...
```{r}
unit_energy <- str_extract(energy_sources, regex("[\\w]*"))
unit_energy
```
...
```{r}
energy_clean <-tibble(source = source_energy, unit = unit_energy)
energy_clean
```

e.  Discussion: Why might regex cleaning be important before performing joins or aggregations?\
    it can isolates the important information we want from massive data, and clean out the only data we need with perfect styling and format.
...
```{r}
left_merged <- left_join(regions, states, by = "state")
nrow(left_merged)
```
...
```{r}
inner_merged <- inner_join(regions, states)
nrow(inner_merged)
```
...
```{r}
anti_join(regions, states, by = "state")
```

d.  Discussion: Why would you prefer a left join over an inner join when merging state-level datasets?\
    left join we can choose which to be left and which we dont want, but inner we cant choose
e. Use pivot_longer() to make the socioeconomic data longer.\

```{r}
a <-pivot_wider(states, names_from = Population, values_from = state)
